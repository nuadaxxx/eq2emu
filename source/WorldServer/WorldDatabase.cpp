...
void WorldDatabase::LoadSpells()
{
	...
	LogWrite(SPELL__INFO, 0, "Spells", "[DIAG] LoadSpells main SELECT status: %s (DB error %u: %s)",
		load_spells_main_select_ok ? "OK" : "FAILED", database_new.GetError(), database_new.GetErrorMsg());

	if( !load_spells_main_select_ok )
	{
	...
	else
	{
		while( result.Next() )
		{
			...
			if(total == 0)
				LogWrite(SPELL__INFO, 0, "Spells", "[DIAG] LoadSpells first iterated row: id=%u tier=%u name='%s'", spell_id, result.GetInt8Str("tier"), spell_name.c_str());
	...
	if(load_spells_main_select_ok && total == 0)
		LogWrite(SPELL__INFO, 0, "Spells", "[DIAG] LoadSpells main SELECT reported OK, but result iteration produced 0 rows.");
